    ...
    return ( 
        <div className='px-6 sm:px-10 pt-6 sm:pt-10 pb-[60px] ' id='work'>
            ...
            <div className="flex items-center justify-center">
              <Slider {...settings} className="w-[100%] sm:w-[80%] flex">
                {projects.map((item)=>(
                  <div key={item.id} className='flex flex-col items-center '>
                    <Link  to={item.link}><img src={item.image} alt='' className='w-full h-full border shadow-md cursor-pointer' /></Link>
                    <div className="my-4 flex flex-col items-center">
                      <Link to={item.link} className='font-bold text-sm hover:text-[#8a2159]'>{item.title}</Link>
                      <p className='text-sm font-medium max-w-[340px] mt-2'>{item.description}</p>
                    </div>
